    ...
    <tbody>
        ...
        @foreach ($accounts as $account)
            ...
            <tr>
                ...
                <td>{{showdateformat($account->start_installment_date,'M-Y')}}</td>
                ...
                @while($tempStartTime <= $endTime)
                    ...
                    @endforeach
                    @if($paid == false)
                        @if((strtotime($account->start_installment_date) <= $tempStartTime) && (empty($account->end_date) || strtotime($account->end_date) >= $tempStartTime))
                            <td> 0 </td>
                            <td> 0 </td>
                        @else
                            <td> N/A </td>
